# VENDORED — do not hand-edit except to refresh from upstream.
#
#   Source:  github.com/cirruslabs/macos-image-templates
#   Path:    templates/vanilla-tahoe.pkr.hcl
#   Rev:     106c086ffa78a0701dd3301646d9a9be71fe7baf (main, 2026-08-28)
#   License: MIT (cirruslabs/macos-image-templates)
#
# Local delta vs upstream: the `ansible` entry was dropped from
# required_plugins — this vanilla build uses only `shell` provisioners, and
# keeping the requirement would force installing a plugin the build never
# loads. Everything else is verbatim.
#
# The boot_command below is RELEASE-SPECIFIC: it types its way through THIS
# macOS release's Setup Assistant, so a newer IPSW usually needs a refreshed
# script. To refresh: fetch the current template for your target release from
# the source repo above, re-apply the ansible-drop delta, and update the Rev
# line + README. See README.md § "Refreshing the vendored template".
packer {
  required_plugins {
    tart = {
      version = ">= 1.16.0"
